function [dummy] = common_filter_beamformer(input)
...
%% SOURCE ANALYSIS USING COMMON FILTER

events = {18 256}; %% change this
n_events = length(events);

sources_common = struct();
...
for event_index = 1:n_events
    ...
    for trigger = event
        event_indices = [event_indices 
                    find(cleaned_data.trialinfo == trigger)];  %#ok<AGROW>
    end
    ...
    sources_common.(field_name) = ft_sourceanalysis(cfg, ...
                                                    powcsds{event_index});
    sources_common.(field_name).pos = sourcemodel.pos; % template
...
save_struct(sources_common, fullpath, overwrite);
