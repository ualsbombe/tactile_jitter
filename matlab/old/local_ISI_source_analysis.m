...
grad = ft_read_sens(fullfile(mr_path, 'tactile_jitter_avg.fif'), ...
                   'senstype', 'meg');
grad = ft_convert_units(grad, 'm');
...
load(fullfile(data_path, 'bar_matrices_beta_0_to_500_ms.mat'))
load(fullfile(data_path, 'std_bar_matrices_beta_0_to_500_ms.mat'))
...
for group_name_index = 1:n_group_names
    ...
    for bar_index = 1:n_bars
        x = (1:n_groups) - group_width/2 + (2*bar_index-1) * group_width / (2*n_bars);
    ...
    title('Local ISI: Beta power: Omission relative to non-stimulation')
%     ylabel([upper(parcellation.cfg.method(1)) parcellation.cfg.method(2:end) ...
%            ' Power increase'])
    ylabel('Mean Power increase')
    ylim([-0.20 0.20])
    xlabel('ROI')
